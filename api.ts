...
export const api = {
  ...
  async verifyOtp(phone: string, code: string, purpose?: string) {
    // ⚠️ إصلاح أمني حرج: كان الفشل هنا (شبكة، مسار خاطئ، إلخ) يُعيد
    // "verified: true" دائماً بسبب "|| true" في نهاية الشرط — أي رمز تحقق
    // (حتى الخاطئ) كان يُقبل كل ما تعذّر الوصول للخادم الحقيقي. الآن: أي
    // فشل في التحقق الفعلي = رفض صريح، لا قبول تلقائي أبداً.
    const { authService } = await import('./authService');
    const res = await authService.verifyOTPAndSignIn(phone, code);
    return { verified: !!res?.success, customToken: res?.customToken };
  },
