...
export default function Home({ setCurrentPage, SYSTEM_CONFIG, ads, homeSections = [], onRefresh }: HomeProps) {
  ...
  const heroImages = useMemo(() => [
    ...
    "/images/bg-dates.jpg",
    "/images/placeholder.jpg",
    "/images/bg-fruits.jpg",
    "/images/bg-vegetables.jpg",
    "/images/bg-hero.jpg",
    "/images/placeholder.jpg",
    "/images/placeholder.jpg",
    "/images/placeholder.jpg",
  ...
  const renderDriverPortal = () => (
    <section key="driver-gateway" className="py-24 md:py-40 relative z-30 bg-slate-950 overflow-hidden border-t-8 border-emerald-600">
      <div className="absolute inset-0 z-0">
        <div className="absolute top-0 left-0 w-[700px] h-[700px] bg-emerald-500/10 rounded-full blur-[150px]"></div>
      </div>
      <div className="container mx-auto px-6 relative z-10 text-center">
        <div className="max-w-5xl mx-auto space-y-8 md:space-y-12">
          <div className="inline-flex items-center gap-4 bg-emerald-500/10 border border-emerald-500/30 px-6 py-3 md:px-10 md:py-5 rounded-full backdrop-blur-3xl">
            <div className="w-2 h-2 md:w-3 md:h-3 bg-emerald-500 rounded-full animate-ping"></div>
            <span className="text-emerald-400 font-black text-xs md:text-lg uppercase tracking-[0.3em]">
              {language === 'ar' ? 'بوابة المناديب والسواقين' : 'DRIVERS & DELEGATES PORTAL'}
            </span>
          </div>

          <h2 className="text-[clamp(2rem,6vw,5rem)] font-black text-white tracking-tighter leading-[1.1]">
            {language === 'ar' ? 'بوابة المناديب والسواقين' : 'Drivers & Delegates Portal'}
          </h2>

          <p className="text-base md:text-2xl text-white/60 font-bold leading-relaxed max-w-3xl mx-auto">
            {language === 'ar'
              ? 'مساحة عمل خاصة بمناديب وسواقين نجوم دلتا المعتمدين لاستلام المهام، تتبع مسار التوصيل، وتوثيق التسليم بتوقيع العميل.'
              : 'A dedicated workspace for verified Delta Stars drivers to receive tasks, track delivery routes, and confirm delivery with customer signature.'}
          </p>

          <motion.button
            whileHover={{ scale: 1.05, y: -4 }}
            whileTap={{ scale: 0.95 }}
            onClick={() => setCurrentPage('driver_login')}
            className="group relative bg-emerald-600 text-white px-10 md:px-20 py-5 md:py-8 rounded-3xl font-black text-xl md:text-3xl shadow-lg hover:bg-emerald-500 transition-all flex items-center justify-center gap-4 md:gap-8 mx-auto border-b-8 border-emerald-800/40 active:border-b-0 active:translate-y-2"
          >
            <span className="text-3xl md:text-5xl group-hover:-rotate-6 transition-transform">🚚</span>
            <span>{language === 'ar' ? 'دخول المندوب / السائق' : 'Driver / Delegate Login'}</span>
          </motion.button>
        </div>
      </div>
    </section>
