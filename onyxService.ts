...
const DEFAULT_CONFIG: OnyxConfig = {
  // ⚠️ أمان: لا تضع مفتاحاً سرياً حقيقياً هنا خلف بادئة VITE_ أبداً — أي متغير
  // ببادئة VITE_ يُضمَّن حرفياً وعلناً داخل حزمة الجافاسكربت المُرسَلة للمتصفح،
  // حتى لو كان اسمه "SECRET". هذه الخدمة حالياً محاكاة (simulation) فقط ولا
  // تنفّذ أي طلب شبكي حقيقي (لا يوجد fetch() في هذا الملف)، لذا لا تسريب فعلي
  // حالياً. لكن عند ربط ERP Onyx حقيقياً مستقبلاً: انقل apiKey/secretKey إلى
  // Netlify Function خلفية (بنفس نمط otp-send.mjs) ولا تستدعِ الـ API مباشرة
  // من العميل أبداً.
  apiUrl: import.meta.env.VITE_ONYX_API_URL || 'https://erp.deltastars.sa/api/v1',
  apiKey: import.meta.env.VITE_ONYX_API_KEY || '',
  secretKey: import.meta.env.VITE_ONYX_SECRET_KEY || '',
  companyCode: '01',
  branchCode: '01',
  warehouseCode: 'WH01',
