  ...
  const renderPageContent = () => {
    switch (currentPage) {
      ...
      case 'driver_login':
        return (
          <DriverLoginPage
            onLoginSuccess={() => handleNavigate('driver_dashboard')}
            onBack={() => handleNavigate('home')}
          />
        );
      case 'driver_dashboard':
        // ⚠️ إصلاح أمني: هذا المسار كان بدون أي تحقق تسجيل دخول إطلاقاً —
        // أي شخص يصل لهذه الحالة (حتى بدون حساب) كان يُعرض عليه مباشرة
        // لوحة المندوب. الآن يتحقق من وجود جلسة دخول فعلية أولاً.
        if (!isAuthenticated) {
          return (
            <DriverLoginPage
              onLoginSuccess={() => handleNavigate('driver_dashboard')}
      ...
      case 'drivers_portal':
        // ملاحظة تصحيح: هذا المسار ليس أداة إدارية للأدمن — هو بوابة دخول
        // ذاتية للمناديب أنفسهم (DriversPortal.tsx لها شاشة دخول داخلية خاصة
        // بها برقم جوال + كلمة مرور/PIN). لذلك لا يُبوَّب هنا خلف دخول أدمن.
        // ⚠️ تنبيه منفصل غير مُصلح بعد: نظام الدخول الداخلي لهذي الشاشة نفسه
        // يعتمد حالياً على تخزين محلي (localStorage) وليس جلسة موقّعة
        // رقمياً مثل ما تم إصلاحه في DriverLoginPage.tsx — يحتاج نفس المعالجة
        // في جلسة قادمة لتوحيد نظام دخول المناديب في مسار واحد آمن.
