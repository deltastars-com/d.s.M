...
const SecuritySection: React.FC<{ addLog?: (msg: string, type: 'info'|'err'|'warn') => void }> = ({ addLog }) => {
  ...
  const handleSavePortalPasses = async () => {
    // ⚠️ إصلاح أمني: تم تعطيل الحفظ الفعلي لهذه الميزة عمداً. كانت تخزّن
    // كلمات مرور كل الأقسام (أدمن، مطور، مستودع، سائقين، B2B، جودة...) كنص
    // صريح غير مشفّر في localStorage وفي مستند Firestore عام (system/credentials)
    // بينما كانت تدّعي للمستخدم "تم الحفظ والتشفير بنجاح" وهذا غير صحيح إطلاقاً.
    // نموذج "كلمة مرور واحدة مشتركة لكل قسم يُدار من المتصفح" غير آمن ببنيته
    // الأساسية بغض النظر عن أي تعديل، لذا تم تعطيله بدل ترقيعه. البديل الآمن
    // الصحيح: حساب دخول فردي حقيقي لكل موظف (بنفس نمط بوابة دخول المناديب
    // التي بُنيت بحساب Firebase حقيقي لكل شخص + صلاحية موقّعة رقمياً)، بدل
    // كلمة مرور واحدة يتشاركها كل قسم.
    addToast(
      language === 'ar'
        ? '⚠️ هذه الميزة معطّلة لأسباب أمنية. كلمات المرور المشتركة للأقسام غير آمنة بطبيعتها — تواصل مع المطور لإنشاء حسابات دخول فردية حقيقية لكل موظف بدلاً منها.'
        : '⚠️ This feature is disabled for security reasons. Shared department passwords are inherently insecure — contact the developer to create real individual login accounts per staff member instead.',
      'error'
    );
    return;
  };
  ...
  const handleResetDefaultPasses = () => {
    // ⚠️ تم تعطيل هذه الميزة لنفس السبب الأمني أعلاه — كانت تعيد كلمات مرور
    // افتراضية ضعيفة جداً وقابلة للتخمين (321666، warehouse123، driver123...)
    // لكل الأقسام دفعة واحدة.
    addToast(
      language === 'ar'
        ? '⚠️ هذه الميزة معطّلة لأسباب أمنية (كانت تعيد كلمات مرور افتراضية ضعيفة جداً).'
        : '⚠️ This feature is disabled for security reasons (it reset to extremely weak default passwords).',
      'error'
    );
    return;
  };
            ...
            >
              ...
              <div className="space-y-6">
                ...
                <div className="flex gap-4">
                  ...
                  <button 
                    onClick={async () => {
                      // ⚠️ إصلاح أمني: كان هذا الفحص يقارن مباشرة بكلمات مرور
                      // مكتوبة كنص صريح بالكود (نفس الباب الخلفي المحذوف من
                      // AuthContext.tsx). بما أن الوصول لهذي الشاشة أصلاً يتطلب
                      // جلسة مطور حقيقية موثّقة عبر Firebase (بعد إصلاح
                      // loginToAdminDashboard)، نعتمد على تلك الجلسة الحقيقية
                      // بدل مقارنة نصوص قابلة للتخمين. التوصية للمستقبل: استبدال
                      // هذا الحقل بإعادة مصادقة فعلية (Firebase reauthenticate)
                      // أو بصمة WebAuthn بدل حقل نصي حر.
                      if (isRole(['developer', 'admin'])) {
                        await executeUpdate();
                      } else {
                        addToast(language === 'ar' ? 'صلاحياتك لا تسمح بهذا الإجراء' : 'You are not authorized for this action', 'error');
                      }
                    }}
                    disabled={isSigning}
...
export const DeveloperDashboard: React.FC<{ onBack?: () => void }> = ({ onBack }) => {
  ...
  const handlePinVerify = async () => {
    // ⚠️ إصلاح أمني: نفس الباب الخلفي (كلمة مرور مكتوبة كنص صريح) الذي أُزيل
    // من الأماكن الأخرى. الوصول لهذي الشاشة أصلاً يتطلب جلسة مطور حقيقية
    // موثّقة، فنعتمد عليها بدل حقل PIN قابل للتخمين. للمستقبل: يُستحسن ربط
    // هذا الزر مباشرة بتسجيل بصمة WebAuthn الحقيقي بدل أي حقل نصي.
    if (isRole(['developer', 'admin'])) {
      setIsBiometricVerified(true);
