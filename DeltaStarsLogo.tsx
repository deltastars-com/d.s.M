...
export const DeltaStarsLogo: React.FC<DeltaStarsLogoProps> = ({ 
  ...
  const primaryUrl = logoUrl || "/official_logo.png?v=2026";
  const [currentSrc, setCurrentSrc] = useState<string>(primaryUrl);
  const [logoFailed, setLogoFailed] = useState<boolean>(false);

  useEffect(() => {
    const url = logoUrl || "/official_logo.png?v=2026";
    setCurrentSrc(url);
    setLogoFailed(false);
  }, [logoUrl]);

  const handleImageError = () => {
    if (currentSrc.includes("/official_logo.png")) {
      setCurrentSrc("/logo.png?v=2026");
    } else if (currentSrc.includes("/logo.png")) {
      setCurrentSrc("/icon-1024.png?v=2026");
    } else if (currentSrc.includes("/icon-1024.png")) {
      setCurrentSrc("/icon-512.png?v=2026");
    } else {
      setLogoFailed(true);
    }
  };
