...
export interface Product {
  ...
  category_en?: string;
  price: number;
  /** تكلفة الشراء/التوريد الفعلية للوحدة — أساسي لحساب تكلفة البضاعة
   *  المباعة (COGS) والربح الإجمالي الحقيقيين. اختياري لأن الكتالوج
   *  الحالي (235 منتج) لا يحتوي هذه القيمة بعد؛ يجب إدخالها فعلياً من
   *  فواتير الموردين الحقيقية قبل الاعتماد على تقارير الربحية. */
  costPrice?: number;
  price_500g?: number;
  price_1kg?: number;
...
export type UserRole = 'admin' | 'ops' | 'developer' | 'gm' | 'vip' | 'client' | 'marketing' | 'delegate' | 'accountant' | 'sales' | 'driver' | 'branch_agent' | 'quality_officer' | 'qa' | 'customer';
...
export interface Order {
  ...
  couponCode?: string;
  onyx_sync_status?: 'pending' | 'synced' | 'failed';
  onyx_invoice_id?: string;
  location?: { lat: number; lng: number };
  latitude?: number;
  longitude?: number;
...
export interface OnyxSyncLog {
  ...
  entityType: 'order' | 'invoice' | 'product' | 'client';
  entityId: string;
  errorMessage?: string;
}
